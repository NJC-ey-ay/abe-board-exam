          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex gap-4 mb-4">
              <div className="bg-green-50 px-4 py-2 rounded-lg">
                <span className="text-green-700 font-medium">{stats.correct} correct</span>
              </div>
              <div className="bg-red-50 px-4 py-2 rounded-lg">
                <span className="text-red-700 font-medium">{stats.wrong} wrong</span>
              </div>
            </div>
            ...
            {currentProblem && (
              <div className="bg-white rounded-2xl shadow-sm border p-6">
                ...
                <div className="text-center mb-8">
                  <p className="text-gray-600 mb-2">Convert:</p>
                ...
                <div className="max-w-xs mx-auto">
                  <input
                    ...
                    className="w-full px-4 py-3 text-center text-xl border-2 border-gray-200 rounded-xl focus:border-primary-500 focus:outline-none"
                    disabled={showAnswer}
                  />
                  <button
                ...
                {showAnswer && (
                  <div className={`mt-6 p-4 rounded-xl ${isCorrect ? 'bg-green-50' : 'bg-red-50'}`}>
                    <div className={`text-center mb-4 ${isCorrect ? 'text-green-700' : 'text-red-700'}`}>
      ...
      {activeTab === 'reference' && (
        <div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {conversionCategories.map((category) => (
              <div key={category.id} className="bg-white rounded-xl shadow-sm border p-5">
                <h3 className="font-semibold text-lg mb-2 text-primary-700">{category.name}</h3>
              ...
              <div className="space-y-4">
                {memoryTechniques.map((technique, index) => (
                  <div key={index} className="bg-white rounded-xl shadow-sm border p-5">
                    ...
                    <p className="text-primary-700 text-sm bg-primary-50 p-2 rounded">
            ...
            <div>
              ...
              <div className="space-y-4">
                {commonConversionGroups.map((group, groupIndex) => (
                  <div key={groupIndex} className="bg-white rounded-xl shadow-sm border p-5">
                    ...
                    <div className="space-y-2 mb-3">
                      {(group as any).conversions?.map((conversion: any, conversionIndex: number) => (
                        <div key={conversionIndex} className="text-sm">
                          ...
                          {'note' in conversion && (
                            <span className="text-gray-500 text-xs ml-2">({(conversion as any).note})</span>
                          )}
                        </div>
                      ))}
                    </div>
